...
def write_health_report(batch_df, batch_id):
    ...
    health_report_df = (
        ...
        .count()
        .withColumn("pipeline_name", lit("mini-kafka-pyspark-data-quality-pipeline"))
...
def main():
    ...
    spark = (
        SparkSession.builder.appName("MiniKafkaPySparkDataQualityPipeline")
        .getOrCreate()
    )
    ...
    clean_query = (
        clean_df.writeStream.format("json")
        .option("path", str(CLEAN_OUTPUT_PATH))
        .option("checkpointLocation", str(CHECKPOINT_PATH / "clean_data"))
        .outputMode("append")
        .start()
    )

    bad_query = (
        bad_df.writeStream.format("json")
        .option("path", str(BAD_OUTPUT_PATH))
        .option("checkpointLocation", str(CHECKPOINT_PATH / "bad_records"))
        .outputMode("append")
        .start()
    )

    health_query = (
        quality_df.writeStream.foreachBatch(write_health_report)
        .option("checkpointLocation", str(CHECKPOINT_PATH / "health_report"))
        .outputMode("append")
        .start()
    )
